...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const savedFavorites = localStorage.getItem('meal_favorites');
  ...
  const saveProfile = (p: UserProfile) => {
    setProfile(p);
    localStorage.setItem('meal_profile', JSON.stringify(p));
  };

  // 保存到历史
  const saveToHistory = (plan: MealPlan) => {
    const newHistory = [plan, ...history].slice(0, 10); // 保留最近10条
    setHistory(newHistory);
    localStorage.setItem('meal_history', JSON.stringify(newHistory));
  };

  // 收藏
  const toggleFavorite = (plan: MealPlan) => {
    const isFav = favorites.some(f => f.cycle_id === plan.cycle_id);
    let newFavorites;
    if (isFav) {
      newFavorites = favorites.filter(f => f.cycle_id !== plan.cycle_id);
    } else {
      newFavorites = [...favorites, plan];
    }
    setFavorites(newFavorites);
    localStorage.setItem('meal_favorites', JSON.stringify(newFavorites));
  };

  const isFavorite = (plan: MealPlan) => favorites.some(f => f.cycle_id === plan.cycle_id);
  ...
  const handleSubmit = async () => {
    if (profile.goals.length === 0 || session.style_preferences.length === 0) { setError('请至少选择一个目标和风格'); return; }
    ...
    try {
      ...
      if (data.error) setError(data.error); else { setResult(data); setSelectedDay(1); saveToHistory(data); }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-amber-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        <h1 className="text-3xl font-bold text-center mb-4">🍽️ 饮食规划助手</h1>
        <div className="text-center mb-6">
          <button onClick={() => setShowHistory(!showHistory)} className="text-sm text-blue-500 hover:underline">
            {showHistory ? '隐藏' : '📜 查看历史记录'} ({history.length})
          </button>
        </div>
        ...
        {showHistory && (
          <div className="bg-white rounded-2xl shadow-lg p-4 mb-6">
            ...
            {history.length === 0 ? <p className="text-gray-400 text-sm">暂无历史</p> : (
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {history.map((h, i) => (
                  <div key={i} className="flex justify-between items-center p-2 bg-gray-50 rounded-lg">
                    <div>
                      <span className="text-sm">{h.session_plan?.style_preferences?.join('、') || '规划'}</span>
                      <span className="text-xs text-gray-400 ml-2">{h.session_plan?.days}天</span>
                    </div>
                    <button onClick={() => { setResult(h); setShowHistory(false); }} className="text-blue-500 text-sm">查看</button>
                  </div>
                ))}
              </div>
            )}
        ...
        {result ? (
          <div className="bg-white rounded-2xl shadow-lg p-6">
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex gap-2">
                <button onClick={() => toggleFavorite(result)} className="px-4 py-2 bg-gray-100 rounded-lg text-sm">
                  {isFavorite(result) ? '❤️ 已收藏' : '🤍 收藏'}
                </button>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input value={customRestriction} onChange={e => setCustomRestriction(e.target.value)} placeholder="添加自定义禁忌..."
                      className="flex-1 px-3 py-2 border rounded-lg text-sm" onKeyDown={e => e.key === 'Enter' && addCustom(customRestriction, profile.custom_restrictions || [], v => saveProfile({...profile, custom_restrictions: v}), setCustomRestriction)} />
                    <button onClick={() => addCustom(customRestriction, profile.custom_restrictions || [], v => saveProfile({...profile, custom_restrictions: v}), setCustomRestriction)} className="px-4 py-2 bg-gray-200 rounded-lg text-sm">添加</button>
                  </div>
                </div>
                <div>
            ...
            <div className="flex justify-between mt-8 pt-4 border-t">
              ...
              <button onClick={() => { setProfile({goals: [], hard_restrictions: [], kitchen_tools: [], custom_restrictions: []}); setSession({style_preferences: [], soft_dislikes: [], custom_dislikes: [], days: 3, meals_per_day: ['早餐','午餐','晚餐'], person_count: 2, budget: '无限制'}); setStep(1); }} className="px-4 py-2 text-gray-400 text-sm">🔄 重置</button>
